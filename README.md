...
- **Логические операторы:** 
    - `&&` и `and` (логическое И): Возвращает `true`, если оба операнда равны `true`, иначе возвращает `false`.
    - `||` и `or` (логическое `ИЛИ`): Возвращает `true`, если хотя бы один из операндов равен `true`, иначе возвращает `false`.
    - `!` и `not` (логическое `НЕ`): Инвертирует значение операнда; если операнд `true`, то возвращает `false`, и наоборот.
- **Операторы сравнения:**
    - `==` (равно): Возвращает `true`, если операнды равны. В противном случае возвращает `false`.
    - `!=` (не равно): Возвращает `true`, если операнды не равны. В противном случае возвращает `false`.
    - `>` (больше): Возвращает `true`, если левый операнд больше правого. В противном случае возвращает `false`.
    - `<` (меньше): Возвращает `true`, если левый операнд меньше правого. В противном случае возвращает `false`.
    - `>=` (больше или равно): Возвращает `true`, если левый операнд больше или равен правому. В противном случае возвращает `false`.
    - `<=` (меньше или равно): Возвращает `true`, если левый операнд меньше или равен правому. В противном случае возвращает `false`.
...
```eye
read(y); ~ принимает значение y и выводит, а после запращивает ввод.
         ~ значения y может и не быть, тогда ничего выводиться не будет.
         ~ возвращает строковое заначение, которое можно преобразовать
         ~ в любое другое значение функциями конвертации. 
```
...
let sub_str = sub_string((str1 + ", " + str2), 7);
println("Подстрока: " + sub_str);  ~ Должно вывести "Подстрока: Мир"
...
#### `sub_string(string, start, end=None)`
...
#### `find_sub_string(string, sub_string)`
Находит первое вхождение подстроки `sub_string` в строку `string` и возвращает его индекс. Если подстрока не найдена, возвращает `-1`.
...
#### `replace_sub_string(string, old_sub_string, new_sub_string)`
Заменяет все вхождения `old_sub_string` в строку `string` на `new_sub_string`.
...
- **Константы:** Хоть переменные и являются константами, но их возможно изменять, тое пересоздать созданием переменной с таким же именем и любым значением. Поэтому нам нужно создать константы запрещающие изменение себя.
...
- **Стандартная библиотека:** Реализовать стандартную библиотеку со всеми включенными в него константами, функциями и т.д.
